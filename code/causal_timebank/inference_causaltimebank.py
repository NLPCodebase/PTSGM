...
from seq2seq_model_causaltimebank import Seq2SeqModel,AutoTokenizer
...
TokenModel = "./exp/causaltimebank"
# TokenModel = "D:/bart-base"
tokenizer = AutoTokenizer.from_pretrained(TokenModel)


model = BartForConditionalGeneration.from_pretrained('./exp/causaltimebank')
...
class_label=['Causilty','Unrelated']
...
plt.show()
# plt.savefig('./figure3-C.eps', format='eps', dpi=1000)

results = {
    
    "F": f1_score(trues_list, preds_list,pos_label="Cause-Effect"),
    "P": precision_score(trues_list, preds_list,pos_label="Cause-Effect"),
    "R": recall_score(trues_list, preds_list,pos_label="Cause-Effect")
